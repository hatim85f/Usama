...
import * as fetchingLocationActions from "../../../common/src/actions/locationactions";
...
export default function SearchScreen(props) {
  const { savedPlaces } = useSelector((state) => state.locationdata);
  const { t } = i18n;
  const isRTL =
    ...
    async (text) => {
      ...
      if (text.length > (settings.AllowCriticalEditsAdmin ? 3 : 5)) {
        ...
        if (savedPlaces.length > 0) {
          Alert.alert(
            "Good",
            `Your Response returning ${savedPlaces.length} resultls`[
              { text: "Ok" }
            ]
          );
